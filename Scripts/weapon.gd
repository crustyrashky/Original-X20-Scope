...
func _ready()->void :
	...
	if player:
		...
		scope_mat = load("res://Imported_Mesh/AR_material2.material")
		scope_camera = get_node_or_null("Player_Weapon/Viewport/Camera")
		scope_mat.albedo_texture = scope_camera.get_viewport().get_texture()
...
func _process(delta)->void :
	...
	if player:
		...
		if current_weapon == W_AR:
			...
			AR_dot.show()
			scope_camera.rotation.y = Global.player.rotation.y + deg2rad(180)
			scope_camera.rotation.x = - Global.player.rotation_helper.rotation.x
			scope_camera.global_transform.origin = global_transform.origin
		...
		if current_weapon != null and not glob.player.psychosis:
			if current_weapon == W_AR:
				Global.player.body_mesh.get_parent().hide()
			else :
				Global.player.body_mesh.get_parent().show()
			...
			if zoom_flag and (current_weapon == W_SNIPER or current_weapon == W_MAUSER):
				...
				Global.player.player_view.fov = lerp(Global.player.player_view.fov, Global.FOV * 0.6, 5 * delta)
			elif zoom_flag:
